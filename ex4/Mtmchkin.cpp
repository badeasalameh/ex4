...
void Mtmchkin::set_in_LeaderBoard(){
    for (auto& m_player: m_players) {
        ...
        m_leaderboard.insert(curPlayer);
    }
}
...
//bool Mtmchkin::playerCompare(const std::shared_ptr<Player>& p1, const std::shared_ptr<Player>& p2){
//    if(p1 -> getLevel() > p2 -> getLevel())
//    {
//        return true;
//    }
...
void Mtmchkin::playRound() {
    ...
    for (auto it = m_players.begin(); it != m_players.end(); it++){
        if (it->second == nullptr) {
            it = m_players.erase(it); // erase and get the next valid iterator
            continue;
        }
...
/////insrer players in the leader board!
//        for (auto & m_player : m_players) {
//            auto curPlayer = m_player.second;
//            m_leaderboard.insert(curPlayer);
//        }
